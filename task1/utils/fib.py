...
def fib(number):
    ...
    В этой функии управляем счетчиком вложенности отсюда, так как
    логика подсчета вложенности в декораторе рассчитана на одиночную
    рекурсию, а здесь за раз вызываем два раза.
    """
